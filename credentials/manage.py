...
def generate_root_cert(name, san):
    ...
    write_cert(f"ca/{name}-ca.pem", cert)
    write_key(f"ca/{name}-privkey.pem", key)
...
def generate_cert(root_name, cert_name, san):
    ...
    root_key = load_key(f"ca/{root_name}-privkey.pem")
    ...
    write_cert(f"cert/{cert_name}-cert-signed-by-{root_name}.pem", cert)
    write_key(f"cert/{cert_name}-privkey.pem", cert_key)
